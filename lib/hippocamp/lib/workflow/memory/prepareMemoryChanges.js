 ...
 */
function __addToMemoryChangeSet (setValues, unsetValues, normalisedFieldName, isASetOperation, value) {
...
function prepareMemoryChanges (
	...
	const allowedSetOperations = [ `set`, `increment`, `decrement` ];
	const setValues = {};
	const unsetValues = {};
	...
	for (const fieldName in memoryDefinitions) {
		...
		this.__addToMemoryChangeSet(setValues, unsetValues, normalisedFieldName, isASetOperation, value);
	}
